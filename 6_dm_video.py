...
imgL = pair_img [0:img_height,0:int(img_width/2)] #Y+H and X+W
imgR = pair_img [0:img_height,int(img_width/2):img_width] #Y+H and X+W
...
imgL = cv2.cvtColor(cv2.resize(cv2.imread("im2.jpg").copy(), (640, 480)), cv2.COLOR_BGR2GRAY)
imgR = cv2.cvtColor(cv2.resize(cv2.imread("im3.jpg").copy(), (640, 480)), cv2.COLOR_BGR2GRAY)

disparity = cv2.divide(stereo.compute(imgL, imgR).astype(np.float32), 255.)
# show the frame
